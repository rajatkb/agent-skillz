<#
.SYNOPSIS
    Search rutor.info for games, pick one, open magnet in qBittorrent.
.DESCRIPTION
    Interactive PowerShell tool: searches rutor.info, displays results with
    size/seeders, lets user pick, opens magnet in qBittorrent via Start-Process.
    Use this when running from Windows — WSL Python cannot connect (TLS issue).
.PARAMETER Q
    Game name to search for (e.g. "cyberpunk 2077").
.PARAMETER F
    Optional filter text (e.g. "InsaneRamZes", "FitGirl", "Portable").
    Case-insensitive substring match against title.
.EXAMPLE
    .\rutor-search.ps1 -Q "elden ring"
    .\rutor-search.ps1 -Q "cyberpunk 2077" -F "InsaneRamZes"
#>
...
if(!$Q){
    ...
    if(!$F){$F = Read-Host "Filter (e.g. InsaneRamZes) or Enter to skip"}
}
if(!$Q){exit}
